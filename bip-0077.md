...
by a `+` character, which must be [RFC 3986
percent-encoded](https://datatracker.ietf.org/doc/html/rfc3986#section-2.1) 
as well as `%2B` to avoid it being interpreted as a space in a Bitcoin URI parameter.
Note that it is important to maintain (uppercase url)[uppercase-url] that `%2B` is used 
instead of `%2b` for QR alphanumeric mode optimization.
